...
function openTerminal(failSafer) {
  ...
  var elixirPath = atom.config.get("elixir-docs.elixirPath")
  var args = !!failSafer ? [] : [path.join((elixirPath || ""), "mix")];
  var command = path.join((elixirPath || ""), "iex")
...
module.exports = ElixirDocs = {
  config: {
    elixirPath:{
      type: 'string',
      default: "",
      description: "Absolute path to elixir executable (required on Mac)"},
    erlangHome:{
      type: 'string',
      default: "",
